...
public class MController {
	...
	@GetMapping("/login/kakaoMemberForm")
	public String kakaoMemberForm() {
		return "login/kakaoMemberForm";
	}
	...
	//		System.out.println("3-2. kakaoUserToken class Nickname  :  "+kakaoDto.getProperties().getNickname());
			...
			MemberDto mDto = memberService.compareId(kakaoId);
			
			if (mDto != null) {
				// 카카오로그인 계정이 DB에 저장되어 있으면 로그인 진행
				...
				// 카카오로그인 계정이 DB에 없으면 회원가입 페이지로 리다이렉트
	            session.setAttribute("sessionId", kakaoId);
	            session.setAttribute("sessionName", nickname);
	            session.setAttribute("session_email", email);
	            
	            return "login/kakaoMemberForm";  // 회원가입 페이지로 이동
			}
